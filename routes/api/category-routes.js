...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    !updated[0] ? res.status(404).json({ message: 'not available' }) : res.status(200).json(updated);
  } catch (err) {
    res.status(500).json({ message: 'not available' });
  }
});
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    !deleted ? res.status(404).json({ message: 'not available' }) : res.status(200).json(deleted);
  } 
  catch (err) {
